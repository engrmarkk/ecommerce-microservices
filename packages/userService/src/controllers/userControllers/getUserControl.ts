...
import { IUserDetailsRequest, apiRes, HttpStatusCodes, HttpStatusMessages } from '@ecommerce/shared';
...
const userDetailsController = async (req: Request, res: Response) => {
    ...
    return apiRes(
      ...
      {email: user.email, firstName: user.firstName, lastName: user.lastName, isActive: user.isActive, lastLogin: user.lastLogin, createdAt: user.createdAt, updatedAt: user.updatedAt}
    );
  };
